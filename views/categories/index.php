<div class="card shadow-sm border-0">
    <div class="card-header bg-white d-flex justify-content-between align-items-center py-3">
        <div>
            <h4 class="fw-bold mb-0">Data As Kategori Po</h4>
    ...
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0">
            <thead class="table text-secondary">
                <tr>
                    <th width="60">#</th>
                    <th data-sort='id' onclick="sortTable('id')" style='cursor:pointer'>
            Id <span class='sort-icon'></span>
           </th>
<th data-sort='kategori' onclick="sortTable('kategori')" style='cursor:pointer'>
            Kategori <span class='sort-icon'></span>
           </th>
<th data-sort='aktif' onclick="sortTable('aktif')" style='cursor:pointer'>
            Aktif <span class='sort-icon'></span>
           </th>
...
function loadData(page = 1){
    ...
    fetch(`?action=categories.index&ajax=1&page=${page}&search=${encodeURIComponent(search)}&sort=${sortField ?? ''}&order=${sortOrder}`)                .then(res => res.json())
        .then(res => {
...
function renderTable(rows, page, limit){
    ...
    if(rows.length === 0){
        ...
        html += `<td>${row.aktif ?? ''}</td>`;

            html += `
                <td class="text-center">
                    <div class="btn-group btn-group-sm">
                        <button onclick="editData(${row.id})" class="btn btn-outline-warning"><i class="bi bi-pencil"></i></button>
                        <button onclick="deleteData(${row.id})" class="btn btn-outline-danger"><i class="bi bi-trash"></i></button>
                    </div>
                </td>
            </tr>`;
        });
...
function openForm(){
    document.getElementById('modalTitle').innerText = 'Tambah As Kategori Po';

    fetch('?action=categories.create&ajax=1')
        .then(res => res.text())
        .then(html => {
...
function editData(id){
    document.getElementById('modalTitle').innerText = 'Edit As Kategori Po';

    fetch(`?action=categories.edit&id=${id}&ajax=1`)
        .then(res => res.text())
        .then(html => {
            document.getElementById('modalBody').innerHTML = html;
...
function deleteData(id){
    ...
    fetch(`?action=categories.delete&ajax=1`, {
        method: 'POST',
...
function bindFormSubmit(){
    ...
    form.addEventListener('submit', function(e){
        ...
        fetch('?action=categories.' + action + '&ajax=1', {
            method: 'POST',
            body: formData
        })
        .then(res => res.json())
