<div class="card shadow-sm border-0">
    <div class="card-header bg-white d-flex justify-content-between align-items-center py-3">
        <div>
            <h4 class="fw-bold mb-0">Data As Ms Satuan</h4>
    ...
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0">
            <thead class="table text-secondary">
                <tr>
                    <th width="60">#</th>
                    <th data-sort='idsatuan' onclick="sortTable('idsatuan')" style='cursor:pointer'>
            Idsatuan <span class='sort-icon'></span>
           </th>
...
function loadData(page = 1){
    ...
    fetch(`?action=satuan.index&ajax=1&page=${page}&search=${encodeURIComponent(search)}&sort=${sortField ?? ''}&order=${sortOrder}`)                .then(res => res.json())
        .then(res => {
...
function renderTable(rows, page, limit){
    ...
    if(rows.length === 0){
        html = `<tr><td colspan="4" class="text-center py-5 text-muted small">Tidak ada data ditemukan</td></tr>`;
    } else {
        rows.forEach((row, i) => {
            html += `<tr><td>${start + i + 1}</td>`;
                    html += `<td>${row.idsatuan ?? ''}</td>`;
        html += `<td>${row.nourut ?? ''}</td>`;

            html += `
                <td class="text-center">
                    <div class="btn-group btn-group-sm">
                        <button onclick="editData(${row.nourut})" class="btn btn-outline-warning"><i class="bi bi-pencil"></i></button>
                        <button onclick="deleteData(${row.nourut})" class="btn btn-outline-danger"><i class="bi bi-trash"></i></button>
                    </div>
                </td>
            </tr>`;
        });
...
function openForm(){
    document.getElementById('modalTitle').innerText = 'Tambah As Ms Satuan';

    fetch('?action=satuan.create&ajax=1')
        .then(res => res.text())
        .then(html => {
...
function editData(id){
    document.getElementById('modalTitle').innerText = 'Edit As Ms Satuan';

    fetch(`?action=satuan.edit&id=${id}&ajax=1`)
        .then(res => res.text())
        .then(html => {
            document.getElementById('modalBody').innerHTML = html;
...
function deleteData(id){
    ...
    fetch(`?action=satuan.delete&ajax=1`, {
        method: 'POST',
...
function bindFormSubmit(){
    ...
    form.addEventListener('submit', function(e){
        ...
        fetch('?action=satuan.' + action + '&ajax=1', {
            method: 'POST',
            body: formData
        })
        .then(res => res.json())
